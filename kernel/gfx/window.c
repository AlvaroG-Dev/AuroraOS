...
void window_destroy(window_t *win) {
  if (!win)
    return;
  if (win->taskbar_item) {
    taskbar_remove_item_ptr(win->taskbar_item); // ← cambio
    win->taskbar_item = NULL;
  }
  if (win->content_buffer)
    kfree(win->content_buffer);
...
void window_redraw_surface(window_t *win) {
  ...
  // 1. Sombras
  gfx_draw_shadow(win->surface, win->surface_w, s_clip, win_local,
                  WIN11_CORNER_RADIUS, WIN11_SHADOW_SIZE);

  // 2. Fondo
  gfx_fill_rounded_rect(win->surface, win->surface_w, s_clip, win_local,
                        WIN11_CORNER_RADIUS, WIN11_WIN_BG);

  if (!(win->flags & WIN_FLAGS_NODECORATION)) {
    rect_t titlebar = {WIN11_SHADOW_SIZE, WIN11_SHADOW_SIZE, win->width,
                       WIN11_TITLEBAR_HEIGHT};
    uint32_t t_color = (win->flags & WIN_FLAGS_FOCUSED)
                           ? WIN11_TITLEBAR_ACTIVE
                           : WIN11_TITLEBAR_INACTIVE;

    gfx_fill_top_rounded_rect(win->surface, win->surface_w, s_clip, titlebar,
                              WIN11_CORNER_RADIUS, t_color);

    gfx_blend_rect(win->surface, win->surface_w, s_clip,
                   (rect_t){WIN11_SHADOW_SIZE,
                            WIN11_SHADOW_SIZE + WIN11_TITLEBAR_HEIGHT - 1,
                            win->width, 1},
                   0x30FFFFFF);

    draw_app_icon(win, win->surface, win->surface_w, s_clip,
                  WIN11_SHADOW_SIZE + 10, WIN11_SHADOW_SIZE + 7);
    gfx_draw_string(win->surface, win->surface_w, s_clip,
                    WIN11_SHADOW_SIZE + 36, WIN11_SHADOW_SIZE + 7, win->title,
                    WIN11_TEXT_PRIMARY, FONT_ID_MONO);
    ...
    gfx_draw_line(win->surface, win->surface_w, s_clip, min_x + 18,
                  WIN11_SHADOW_SIZE + 16, min_x + 28, WIN11_SHADOW_SIZE + 16,
                  WIN11_TEXT_PRIMARY);
    gfx_draw_rect_outline(win->surface, win->surface_w, s_clip,
                          (rect_t){max_x + 18, WIN11_SHADOW_SIZE + 11, 10, 10},
                          WIN11_TEXT_PRIMARY);
    gfx_draw_line(win->surface, win->surface_w, s_clip, close_x + 18,
                  WIN11_SHADOW_SIZE + 11, close_x + 27, WIN11_SHADOW_SIZE + 20,
                  WIN11_TEXT_PRIMARY);
    gfx_draw_line(win->surface, win->surface_w, s_clip, close_x + 18,
                  WIN11_SHADOW_SIZE + 20, close_x + 27, WIN11_SHADOW_SIZE + 11,
                  WIN11_TEXT_PRIMARY);
