...
static void format_timestamp(char *buf, size_t buflen) {
  ...
  if (secs == 0) {
    ...
    uint64_t s = secs;
    while (s > 0) { sbuf[sn++] = '0' + (s % 10); s /= 10; }
  }
  while (sn < 5) sbuf[sn++] = ' ';
  while (sn > 0) tmp[n++] = sbuf[--sn];
  ...
  if (usec == 0) {
    ...
    uint64_t u = usec;
    while (u > 0) { ubuf[un++] = '0' + (u % 10); u /= 10; }
  }
  while (un < 6) ubuf[un++] = '0';
  while (un > 0) tmp[n++] = ubuf[--un];

  int i = 0;
  for (int j = 0; j < n && i < (int)buflen - 1; j++) buf[i++] = tmp[j];
  buf[i] = '\0';
}
...
static void emit_prefix(klog_level_t level) {
  ...
  out_char('[');
  for (char *p = ts; *p; p++) out_char(*p);
  out_char(']');
  out_char(' ');
  const char *name = level_name(level);
  for (const char *p = name; *p; p++) out_char(*p);
  out_char(' ');
}
...
static void emit_signed(int64_t v, int base, int min_digits) {
  if (v < 0) { out_char('-'); emit_num((uint64_t)(-v), base, min_digits); }
  else emit_num((uint64_t)v, base, min_digits);
}

// ---------------------------------------------------------------------------
// vprintf (sin lock — el llamante lo tiene)
// ---------------------------------------------------------------------------
static void klog_vprintf_locked(klog_level_t level, const char *fmt, va_list ap) {
  emit_prefix(level);
  ...
  while (*fmt) {
    ...
    while (*fmt == 'l') { if (is_long) is_llong = 1; is_long = 1; fmt++; }
    if (*fmt == 'z') { is_size = 1; fmt++; }

    switch (*fmt) {
      case 'd': case 'i': {
        int64_t v;
        if (is_llong || is_long) v = va_arg(ap, int64_t);
        else if (is_size)        v = (int64_t)va_arg(ap, size_t);
      ...
      case 'u': {
        uint64_t v;
        if (is_llong || is_long) v = va_arg(ap, uint64_t);
        else if (is_size)        v = va_arg(ap, size_t);
...
void klog_vprintf(klog_level_t level, const char *fmt, va_list ap) {
  if (!klog_ready || level > min_level) return;
...
void klog_puts(klog_level_t level, const char *s) {
  if (!klog_ready || !s || level > min_level) return;
...
void klog_calibrate_tsc(uint32_t ticks_to_wait, uint32_t pit_hz) {
  if (ticks_to_wait == 0 || pit_hz == 0) return;

  // Esperar a que empiece un tick nuevo
